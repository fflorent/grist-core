...
describe('scrubUserFromOrg', function() {
  ...
  beforeEach(async function() {
    ...
    const user1 = (await server.dbManager.getUserByLoginOrCreate('user1@getgrist.com'))!;
  ...
  async function getApi(userName: string, orgName: string) {
    const user = (await server.dbManager.getUserByLoginOrCreate(`${userName}@getgrist.com`))!;
